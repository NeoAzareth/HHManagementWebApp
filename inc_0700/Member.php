...
Class Member{
	...
	Private $userBills = [];
	Private $userStatus;
	Private $prefContactId;
	Private $userContactOptions = [];
	...
	function __construct3($userID, $month, $pdo){
		//prepare statement
		$getUserInfo =
		$pdo->prepare('SELECT UserID, LastName, FirstName, UserLevel, UserStatus,
				PreferredNotID
							FROM sp16_users
							WHERE UserID =:userid');
		//execute statement
		...
		$this->prefContactId = $info["PreferredNotID"];
	...
	function getUserBills($month){
		...
		$pdo->prepare('SELECT BillID, BillAmount, BillDesc, BillCategory, BillDate
				FROM sp16_bills
				WHERE UserID =:userid AND (LOCATE(:month,BillDate)!=0)');
